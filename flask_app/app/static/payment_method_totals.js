...
function preencherDivMethodTotals(data) {
  // Obtém a referência da div no HTML onde deseja adicionar os elementos
  const div = document.getElementById("div_payment_method_totals");
  ...
  data.forEach(item => {
    if (item.total_value != 0) {
      ...
      smallElement.textContent = `${item.method_name}: R$ ${item.total_value.toFixed(2)}`;
...
function fetchDataAndPopulateDivMethodTotals() {
  // URL da API
  const apiUrl = '/transactions/payment_method_totals';
...
window.addEventListener('load', fetchDataAndPopulateDivMethodTotals);
